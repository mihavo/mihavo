## Hi, I'm Michael :wave:

Backend engineer focused on system architecture, distributed systems, and cloud.
Currently working on distributed backend services with Spring and exploring patterns in enterprise-scale system design.

---

### Projects

- [**Beacon**](https://github.com/mihavo/beacon) — real-time location sharing and geofencing platform
- [**Nextfin**](https://github.com/mihavo/nextfin) — backend platform for e-banking operations, built around security and scale
- [**Atlacity**](https://atlacity.vercel.app) — full-stack place discovery app, inspired by [Foursquare](https://foursquare.com/city-guide)

---

### GitHub Stats

![Michael's GitHub stats](https://github-readme-stats-sigma-five.vercel.app/api?username=mihavo&show_icons=true&hide_title=true&include_all_commits=true&count_private=true&theme=algolia)

